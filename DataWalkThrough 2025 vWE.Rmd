---
title: "Climate Change Data - An Introduction to R"
...
![Image with settings for data download](emdat_settings.png){width="50%"}
...
![](images/set%20working%20directory.png){width="400"}
...
![](images/New%20scipt.png){width="300"}
...
You can immediately see from the Environment tab that the object has 17,288 observations (or rows) and 46 variables (or columns).
...
![A tweet seen on 20 October 2021](tweet_group_by.png){width="50%"}
...
![Image with error message](error1.png){width="50%"}```{r, echo = FALSE}
...
![Screenshot of EDGAR webpage](EDGARlink.png){width="50%"}
...
![Screenshot of EDGAR data in EXCEL](EDGAR_EXCEL_screenshot.png){width="50%"}
...
edgardat.long <- edgardat %>%
  ...
  mutate(
    Year = sub("Y_", "", Year)         # remove the "Y_" prefix
  )
...
popdat.long <- popdat %>%
  ...
  mutate(
    Year = sub("X", "", Year)         # remove the "X" prefix
  )
```
...
popdat.long <- popdat %>%
  ...
  mutate(
    Year = sub(XXXX, "", XXXX)         # remove the "X" prefix
  )
```
